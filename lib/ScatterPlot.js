...
function Stici_Scatterplot(container_id, params) {
  ...
  this.chartDimensions = { 'width': '600', 'height': '340' };
  this.bottomControls = {
    'sds': false,
    'sd-line': false,
    'avg-graph': false,
  ...
  if (!params.data instanceof Array) {
    ...
    this.chartDimensions = {
      'width': this.options.chartDimensions[0],
      'height': this.options.chartDimensions[1]
    };
    this.chartMargins = {
    ...
    this.chartWidth   = chartDimensions.width;
    this.chartHeight  = chartDimensions.height;
  }
  ...
  this.setScale = function(data) {
    ...
    yScale = d3.scale.linear()
              .domain([yMin, yMax])
              .range([chartHeight - chartMargins.top - chartMargins.bottom, 0])
              .nice();
  ...
  this.plotSelectedOptions = function(discardOptions) {
    ...
    for (var option in bottomControls) {
      if (discardOptions.indexOf(option) > -1) {
        continue;
      }
      tempSelected = bottomControls[option];
  ...
  function initControls() {
    ...
    $.each(this.options.data, function(i, dataUrl) {
    ...
    self.chartDiv = $('<div/>').addClass('stici_chart').attr('id', 'scatterplot');
  ...
  function handleBtnToggle(e) {
    ...
    if (btnId === 'clear-points' || btnId === 'list-data' || btnId === 'univar-stats') {
      ...
      bottomControls[btnId] = (bottomControls[btnId] === true ? false : true);
    }
