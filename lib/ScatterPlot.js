...
//  // 2) Manual specification of data x and y values (NOT IMPLEMENTED).
...
function Stici_Scatterplot(container_id, params) {
  ...
  // anything else.
  if (!dataIsGenerated() && !dataIsFromExternalFile()) {
  ...
  function initControls() {
    ...
    if (dataIsFromExternalFile()) {
      ...
      // r controls
      var rInput = jQuery('<input type="text" />').change(function() {
        rSlider.slider('value', rInput.val());
        self.r = rInput.val();
        loadGeneratedData();
      });
      var updateRInput = function() {
        rInput.val(rSlider.slider('value'));
      ...
      rInput.val(self.r);
      rSlider.slider('value', rInput.val());
      $topControls.append('r: ').append(rInput).append(rSlider);
