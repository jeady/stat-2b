...
function refreshStatsBox() {
  ...
  if (self.options.statisticType == SUM) {
    ...
    se = fpc*self.popSd*Math.sqrt(self.options.sampleSize + 0.0);
  ...
  if (self.options.statisticType == CHISQUARE) {
    ...
    popMeanText = "Ave(Box): " + Number(self.popMean).toFixed(3);
    popSdText = "Sd(Box): " + Number(self.popSd).toFixed(3);
  }
...
function redrawChart() {
      ...
      var currentBinEnds = histMakeBins(self.options.bins, [self.xMin, self.xMax]);
