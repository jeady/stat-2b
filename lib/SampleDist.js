...
function Stici_SampleDist(container_id, params) {
  ...
  function refreshSelectedAreaOverlay() {
    ...
    var left = (lower - self.binEnd.min()) * scale;
    var right = (upper - self.binEnd.min()) * scale;
    ...
    if (self.sampleBinCounts !== null) {
      p = histHiLitArea(lower, upper, self.binEnd, self.sampleBinCounts);
    }
    p *= 100;
  ...
  function setAreas() {
    areaLabel.setText(" Selected area: " + Format.doubleToPct(histHiLitArea(hiLiteLo, hiLiteHi, binEnd, counts)));
    // var type = curveChoice.getSelectedItem()
    if (type == "Normal Curve") {
    // var replacement = replaceCheck.getState();
      curveAreaLabel.setText(" Normal approx: "
                             + Format.doubleToPct(normHiLitArea(hiLiteLo, hiLiteHi, sampleSize, nPop, EX, SE, replacement)));
    } else if (type == "Student t Curve") {
      curveAreaLabel.setText(" Student t approx: "
                             + Format.doubleToPct(tHiLitArea(hiLiteLo, hiLiteHi, )));
    } else if (type == "Chi-Squared Curve") {
      curveAreaLabel.setText(" Chi-squared approx: "
                             + Format.doubleToPct(chiHiLitArea(hiLiteLo, hiLiteHi, )));
    } else {
