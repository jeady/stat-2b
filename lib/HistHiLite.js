...
function Stici_HistHiLite(container_id, params) {
  ...
  function initControls() {
    ...
    statsButton.button.click(function() {
      ...
      text += 'Mean: ' + mean(self.data).toFixed(2) + '<br />';
      text += 'SD: ' + sd(self.data).toFixed(2) + '<br />';
      text += 'Min: ' + self.data.min().toFixed(2) + '<br />';
      text += 'LQ: ' + percentile(self.data, 25).toFixed(2) + '<br />';
      text += 'Median: ' + percentile(self.data, 50).toFixed(2) + '<br />';
      text += 'UQ: ' + percentile(self.data, 75).toFixed(2) + '<br />';
      text += 'Max: ' + self.data.max().toFixed(2) + '<br />';
      statsButton.content.html(text);
    });
    var normalButton = jQuery('<button/>').text('Show Normal Curve');
    row2.append(listDataButton).append(statsButton.parent).append(normalButton);
