...
function Stici_HistHiLite(container_id, params) {
  ...
  function initControls() {
    ...
    function refreshSelectedAreaOverlay() {
      ...
      self.areaInfoDiv.text('Selected area: ' + p.fix(2) + '%');
    }
    ...
    normalButton.click(function() {
      ...
      var oldText = normalButton.text();
      if (oldText.replace('Show', 'Hide') != oldText)
        normalButton.text(oldText.replace('Show', 'Hide'));
      else
        normalButton.text(oldText.replace('Hide', 'Show'));
    });
