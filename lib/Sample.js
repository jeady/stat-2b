    ...
    */
    ...
    function init() {
      ...
      function createPopulationBox() {
        ...
        self.boxLabel = jQuery('<div/>').text('Population');
    ...
    function setBox(newBox, updateBox) {               // parse new population
      ...
      if (newBox.toLowerCase() == "normal") {
        ...
        if (updateBox) {
          self.box.text(jQuery.map(self.pop, function(v) {return v.fix(nDigs);}).join(' '));
        }
    ...
    function setBins(nBins, xMin, xMax, nPop, sourceType, varType, samplesSoFar) {
        ...
        if (samplesSoFar() > 0 ) {                                                      // stub
            if (currVar() == "Sample S-Squared" || currVar() == "Sample Chi-Squared") {    // stub
