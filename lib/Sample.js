function Stici_SampleDist(container_id, params) {
    ...
    */
    ...
    function drawSample(nSams, sampleSize, sourceType, varType) {
        ...
        for (var j=0; j < nSams; j++) {
            ...
            if ( varType == "Sample Chi-Squared")  {
                ...
                } else if (currVar == "Sample S-Squared") {
                    if (ssq < xMin || ssq > xMax) {
                        xMin = Math.min(ssq, xMin);
                        xMax = Math.max(ssq, xMax);
                    }
    ...
    function setBins(nBins, xMin, xMax, nPop, sourceType, varType, samplesSoFar) {
        ...
        if (samplesSoFar > 0 ) {
            if (currVar == "Sample S-Squared" || currVar.equals("Sample Chi-Squared")) {
                ...
                countSample = listToHist(scalVMult(sampleSize,sampleMean), samplesSoFar);
            }
        } else {
            for (var i=0; i < nBins; i++) {
                countSample[i] = 0;
    ...
    function setLims(xMin, xMax, sampleSize, pop, popMin, popMax, popMean, popSd, currVar, showBoxHist) {
        if (currVar == "Sample Sum") {
            xMin = sampleSize * popMin; // these are the limits for the histogram
            xMax = sampleSize * popMax;
        } else if (currVar == "Sample Chi-Squared") {
            xMin = 0.0;
    ...
    function adjustSampleSize(minSampleSize, maxSampleSize, maxMaxSampleSize, sampleSize, sampleSizeBar, nPop, replaceCheck, currVar) {
        minSampleSize = 1;
        if (currVar == "Sample S-Squared" || currVar == "Sample t") {
            minSampleSize = 2;
        }
        if ( !replaceCheck.getState() ) {
            maxSampleSize = nPop;
    ...
    function setCurve(xMin, xMax, pop, popMean, popSd, nPop, sampleSize, xVal, yVal, nx, sd, mu, replaceCheck, showCurve, curveType, varType) {
        var fpc = 1.0;
        ...
        xVal = new Array(nx);
        yVal = new Array(nx);
        if (curveType == "No Curve") {
            showCurve = false;
        } else {
            showCurve = true;
            if (curveType == "Chi-Squared Curve") {
                if (varType == "Sample S-Squared") {
                ...
                } else if (varType == "Sample Chi-Squared") {
                    for (var i=0; i < nx; i++) {
                        xVal[i] = xMin + i*(xMax - xMin)/(nx-1.0);
            ...
            } else if (curveType == "Normal Curve") {
                if (varType == "Sample Mean") {
                    sd = fpc*popSd/Math.sqrt(sampleSize + 0.0);
                    mu = popMean;
                } else if (varType == "Sample Sum") {
                    sd = fpc*popSd * Math.sqrt(sampleSize + 0.0);
                    mu = popMean * sampleSize;
                } else if (varType == "Sample S-Squared") {
            ...
            } else if (curveType == "Student t Curve") {
                if (varType == "Sample t") {
                    for (var i = 0; i < nx; i++) {
                        xVal[i] = xMin + i*(xMax - xMin)/(nx-1);
