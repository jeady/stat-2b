function Stici_SampleDist(container_id, params) {
    ...
    self.varChoice = new SticiComboBox();   // options for which random variable to sample
    self.curveChoice = new SticiComboBox(); // options for which approximating curve to plot
    self.sourceChoice = new SticiComboBox(); // options for data source (box, normal, uniform)
    ...
    */
    ...
    function init() {
      ...
      // Build the middle pieces.
      var statsBox = jQuery('<div/>').addClass('statsText');
      var popText = jQuery('<textarea/>').addClass('population');
      middle.append(statsBox, self.hist, popText);

      statsBox.text('Lorem ipsum');
      ...
      self.hist.width(middle.width() - statsBox.width() - popText.width() - 8);
    }
