function Stici_SampleDist(container_id, params) {
    ...
    */
    ...
    function init() {
      ...
      hist.width(middle.width() - stats.width() - box.width() - 8);
    ...
    function initPop() {
        if (sourceChoice.selected() == "Box") {
            ...
            if (pop.length === 0) {
                ...
                for (var i= 0; i < self.binBar.val(); i++) {
                    countPop[i] = 0;
                }
...
// reset the labels
        if (varChoice.selected() == "Sample Chi-Squared") {
            ...
            popMeanLabel.text("Ave(Box): " + popMean.fi(3));
    ...
    function setBins() {
      binEnd = [];
      jQuery.each(range(0, binBar.val() + 1), function(i) {
          binEnd[i] = xMin + i*(xMax - xMin)/binBar.val();
      });
      countPop = new Array(binBar.val());
      countSample = new Array(binBar.val());
      if (sourceChoice.selected() == "Box" && pop.length > 0) {
        if (varChoice.selected() == "Sample Chi-Squared") {
          setCurve();
        } else {
          countPop = listToHist(pop, pop.length);
          setCurve();
        }
      } else if (sourceChoice.selected() == "Normal") {
        jQuery.each(range(0, binBar.val()), function(i) {
