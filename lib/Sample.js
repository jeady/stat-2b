    ...
    function chiHiLitArea() {
       ...
       if (hiLiteHi > hiLiteLo) {
           if (varChoice.selected() == "Sample S-Squared") {
               ...
               area = chi2Cdf(scale*hiLiteHi, sampleSize-1) -
                      chi2Cdf(scale*hiLiteLo, sampleSize-1);
           } else if (varChoice.selected() == "Sample Chi-Squared") {
               area = chi2Cdf(hiLiteHi, pop.length-1) -
                      chi2Cdf(hiLiteLo, pop.length-1);
           } else {
    ...
    function tHiLitArea() {
       ...
       if (hiLiteHi > hiLiteLo) {
           if (varChoice.selected() == "Sample t") {
               area = tCdf(hiLiteHi, sampleSize-1) -
                      tCdf(hiLiteLo, sampleSize-1);
           } else {
