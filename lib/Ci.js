...
function Stici_Ci(container_id, params) {
    ...
    var sampleMean = null; // the history of sample means
    var sampleSe = null;   // the history of sample SE(mean)'s
    var seUsed = null;     // vector of SE's to use for intervals
    ...
    function xBar() {
      ...
      if (sourceChoice.selected() == "Box" ||
        ...
        if (replaceCheck.checked()) {
          ...
          var samInx = PbsStat.listOfDistinctRandInts(sampleSize, 0, nPop-1);
          for (i = 0; i < sampleSize; i++) {
            x[i] = pop[samInx[i]];
            xb += x[i];
          }
